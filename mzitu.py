...
class Mzitu:
    ...
    def __init__(self):
        ...
        try:
            ...
            if select.lower() == 'q':
                exit("Info: 退出成功！")
            if int(select) not in [0, 1, 2, 3, 4]:
                exit(self.red + "Error: 输入错误！")
    ...
    def getIndexInfo(self, url):
        ...
        urlList = re.compile(
            "https:\/\/www.mzitu.com\/\d+").findall(response.text)
        # 去重
        new = []
        for id in urlList:
            if id not in new:
    ...
    def main(self):
        ...
        maxLen = 99999
        inputMaxlen = input(self.green + "Command: 输入要下载的组数，默认为不限制: ")
        if inputMaxlen != '':
        ...
        for i in range(1, indexPage+1):
            ...
            for j in range(1, len(indexUrls[1])):
                ...
                # print(j)
                if j > maxLen:
                    print(self.green, "超过设定最大下载数 %d 进程结束" % int(maxLen))
                    exit()
                ...
                for k in tqdm(range(1, info[1] + 1)):
                    po.apply_async(func=self.getImage, args=(
                        indexUrls[1][j] + "/" + str(k), k,))
                    time.sleep(0.4)
